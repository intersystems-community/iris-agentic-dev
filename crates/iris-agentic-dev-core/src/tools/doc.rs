...
async fn do_write(
    ...
    // visible error. Callers must pass allow_storage_regeneration=true to proceed.
    if storage_stripped && !allow_storage_regeneration {
        return err_json(
            ...
            "The class content includes an explicit Storage definition. Writing it via \
             ...
             allow_storage_regeneration: true. To edit this class safely, use iris_doc \
             mode=insert or mode=delete_lines to make targeted edits that do not touch \
             the Storage block.",
        );
    }
