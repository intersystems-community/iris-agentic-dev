...
- **`ToolCategory`'s security taxonomy is incomplete and is explicitly out of scope for this spec to backfill.** 54 of the 81 Baseline tools currently have no `ToolCategory` mapping and fall through to "not gated" in the env-template policy check. This is a real gap, but assigning categories to 54 tools is a security-policy judgment call for the tools' owners, not a mechanical fix — a bulk allowlist-style backfill done without that review would create false confidence rather than close the gap. A follow-up categorization pass is recommended before (or alongside) any work that leans on `ToolCategory` for packaging decisions.
